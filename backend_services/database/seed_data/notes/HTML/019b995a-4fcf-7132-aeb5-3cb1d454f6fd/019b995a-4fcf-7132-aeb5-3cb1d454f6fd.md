...
# Latihan
```quiz
 { "question": "1. Apakah fungsi utama bahasa HTML (Hypertext Markup Language) dalam pembinaan sesebuah laman web?", "options": [ "Menghasilkan animasi yang kompleks dan permainan dalam talian.", "Menghuraikan kandungan dan struktur dokumen laman web.", "Menyambungkan pangkalan data terus ke pelayan tanpa skrip.", "Menguruskan perkakasan komputer pengguna semasa melayari internet." ], "correctIndex": 1 }
```
```quiz
{ "question": "2. Di dalam struktur asas HTML, manakah antara berikut merupakan contoh bagi 'Empty Element' (elemen yang tidak mempunyai tag penutup)?", "options": [ "<html>", "<title>", "<body>", "<br>" ], "correctIndex": 3 }
```
```quiz
{ "question": "3. Sekiranya seorang pereka web ingin memaparkan tajuk yang paling besar dan paling penting, tag manakah yang patut digunakan?", "options": [ "<h6>", "<header>", "<h1>", "<h3>" ], "correctIndex": 2 }
```
```quiz
{ "question": "4. Atribut 'src' di dalam elemen <img> digunakan untuk tujuan:", "options": [ "Menetapkan saiz lebar (width) sesuatu imej.", "Menukar warna latar belakang imej kepada lut sinar.", "Menentukan URL atau lokasi fail imej yang ingin dipaparkan.", "Menambah baris baharu di bawah imej tersebut." ], "correctIndex": 2 }
```
```quiz
  { "question": "5. Apakah kegunaan elemen <iframe> di dalam kod HTML?", "options": [ "Untuk menyusun teks dalam bentuk perenggan yang panjang.", "Untuk memaparkan dokumen HTML lain atau laman web luar di dalam laman web semasa.", "Untuk memaparkan logo dan navigasi utama di bahagian atas laman.", "Untuk menetapkan jenis fon yang digunakan dalam keseluruhan dokumen." ], "correctIndex": 1 }
```