import 'dart:io';
import 'package:shelf/shelf.dart';
import 'package:shelf/shelf_io.dart' as io;
import 'package:shelf_static/shelf_static.dart';
import 'package:code_play/utils/local_asset_server_api.dart';

class LocalAssetServer {
  HttpServer? _server;
  int _port = 0;
  LocalAssetServerApi? _apiHandler;

  int get port => _port;
  
  /// Set API handler for Unity HTTP requests (optional)
  void setApiHandler(LocalAssetServerApi apiHandler) {
    _apiHandler = apiHandler;
  }

  Future<void> start({String path = 'assets', int port = 0}) async {
    String servePath = path;

    if (!Directory(servePath).existsSync()) {
      if (Directory('data/flutter_assets/$path').existsSync()) {
        servePath = 'data/flutter_assets/$path';
      } else if (Directory('build/flutter_assets/$path').existsSync()) {
        servePath = 'build/flutter_assets/$path';
      } else {
        print('WARNING: Could not find assets directory at $path.');
      }
    }

    final staticHandler = createStaticHandler(
      servePath,
      defaultDocument: 'index.html',
      listDirectories: true,
    );

    // Use API handler if provided, otherwise use basic CORS middleware
    final Handler handler;
    if (_apiHandler != null) {
      handler = _apiHandler!.createApiHandler(staticHandler);
    } else {
      handler = Pipeline()
          .addMiddleware((innerHandler) {
            return (request) async {
              final response = await innerHandler(request);
              return response.change(
                headers: {
                  'Access-Control-Allow-Origin': '*',
                  'Access-Control-Allow-Methods': 'GET, POST, OPTIONS',
                  'Access-Control-Allow-Headers': '*',
                  'Cross-Origin-Opener-Policy': 'same-origin',
                  'Cross-Origin-Embedder-Policy': 'require-corp',
                },
              );
            };
          })
          .addHandler(staticHandler);
    }

    try {
      _server = await io.serve(handler, InternetAddress.loopbackIPv4, port);
      _port = _server!.port;
      print('LocalAssetServer running on http://localhost:$_port');
      if (_apiHandler != null) {
        print('API endpoints available at http://localhost:$_port/api/');
      }
    } catch (e) {
      print('Failed to start LocalAssetServer: $e');
      rethrow;
    }
  }

  Future<void> stop() async {
    await _server?.close(force: true);
    _server = null;
    print('LocalAssetServer stopped.');
  }
}
